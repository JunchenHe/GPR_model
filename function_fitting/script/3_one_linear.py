...
for input_dim in input_set:
    for train_num in train_num_set:
        ...
        for x in data.train.X:
            pre_y_train.append([func(x,regr.coef_[0],regr.coef_[1],regr.coef_[2],regr.intercept_)])
        pre_y_train=np.array(pre_y_train)


        pre_y_test=[]
        for x in data.test.X:
            pre_y_test.append([func(x,regr.coef_[0],regr.coef_[1],regr.coef_[2],regr.intercept_)])
