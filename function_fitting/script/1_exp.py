...
for input_dim in input_set:
    for train_num in train_num_set:
        ...
        xdata=data.train.mean_and_std_X[:,0]
        ydata=data.train.mean_and_std_Y[:,0]
        ...
        pre_y_train=[]
        for x in data.train.mean_and_std_X:
            pre_y_train.append(func(x,popt[0],popt[1],popt[2]))
        pre_y_train=np.array(pre_y_train)
        data.train.re_mean_and_std(pre_y_train)


        pre_y_test=[]
        for x in data.test.mean_and_std_X:
            pre_y_test.append(func(x,popt[0],popt[1],popt[2]))
        pre_y_test=np.array(pre_y_test)
        data.test.re_mean_and_std(pre_y_test)
        ...
        print(err.calc_rmse(data.test.Y,data.test.predict_Y),end='\t')   

        print(err.calc_err_rate(data.test.Y,data.test.predict_Y),end='\t')
        print(popt,end="")
